...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const rawBaseUrl =
    process.env.SITE_URL || "https://www.kalolwala.com";
  ...
  const staticAndOfferingRoutes = routes.map((route) => {
    const formattedPath = route === "/" ? "/" : route;

    return `${baseUrl}${formattedPath}`;
  });

  const localBlogRoutes = BLOG_DATA
  .filter((post) => post.slug)
  .map((post) => `${baseUrl}/blogs/${post.slug}`);

const wordpressBlogRoutes = wordpressPosts
  .filter((post: any) => post.slug)
  .map((post: any) => `${baseUrl}/blogs/${post.slug}`);
