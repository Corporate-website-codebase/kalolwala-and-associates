...
export default function BlogPaginatedList({ cards = BLOG_DATA }: { cards?: BlogPost[] }) {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage < 1 || newPage > totalPages || newPage === currentPage || isPageChanging) return;
  ...
  return (
    <section className="w-full py-14 bg-[#050505] text-white min-h-screen flex flex-col font-noto-sans">
      <div className="marginal mx-auto w-full px-6 md:px-0">
        ...
        <div className="min-h-[400px]">
           ...
           >
            {currentData.map((c, index) => (
              <a href={c.url} key={c.id} target="_blank" rel="noopener noreferrer" className="group block w-full outline-none">
                <article 
                  className={`
        ...
        {/* --- MINIMAL PAGINATION --- */}
        {totalPages > 1 && (
          <div className={`mt-8 flex justify-between items-end border-t border-white/10 pt-8 transition-opacity duration-1000 ${hasMounted ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="font-mono text-sm text-gray-600">
              0{currentPage} <span className="mx-2 text-gray-800">/</span> 0{totalPages}
            </div>
