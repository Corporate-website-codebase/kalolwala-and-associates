...
//     // Boundary Checks
//     if (newIndex < 0 || newIndex >= teamData.length) return;

//     const newMember = teamData[newIndex];

//     // IMPORTANT: Update Start Rect so if user closes, it goes to the NEW card
//     const cardWrapper = cardsRef.current[newIndex];
//     const cardElement = cardWrapper?.querySelector('.card-container') as HTMLElement;
//     if (cardElement) {
//       const rect = cardElement.getBoundingClientRect();
//       setStartRect(rect);
...
//             </div>
// {activeIndex !== null && (
//               <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-white/10 opacity-0 animate-in fade-in duration-500" style={{ opacity: 1 }}>
                
//                 {/* PREVIOUS BUTTON */}
//                 {activeIndex > 0 && (
//                    <button
//                    onClick={(e) => { e.stopPropagation(); handleNavigate('prev'); }}
//                    className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-full transition-all duration-300 border border-white/10 hover:border-yellow-500/50 group"
//                  >
